...
describe('Pdo', function () {
    ...
    it('should produce a PDO object', function (done) {
        ...
        const obj0005 = device.od.getEntry(0x5);
    ...
    it('should emit on consuming a PDO object', function (done) {
        const device = new Device({ id: 0xA, loopback: true });
        const obj0002 = device.od.getEntry(0x2); // INT8
        const obj0005 = device.od.getEntry(0x5); // UINT8
    ...
    it('should listen to ObjectDictionary#newTpdo', function (done) {
        const device = new Device({ id: 0xA, loopback: true });
        device.pdo.start();
        device.nmt.startNode();
    ...
    it('should listen to ObjectDictionary#removeTpdo', function () {
        const device = new Device({ id: 0xA, loopback: true });
        const obj0005 = device.od.getEntry(0x5);
        device.pdo.start();
        ...
        device.od.removeTransmitPdo(0x18A);
