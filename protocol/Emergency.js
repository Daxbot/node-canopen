...
class Emergency extends EventEmitter
{
    parse(message)
    {
        let code = message.data.readUInt16LE(0);
        let reg = message.data[2];
        let bit = message.data[3];
        let info = message.data.slice(4);

        let parsed = this._parseCode(code);

        if(parsed == 'Error Reset')
            this.emit(code, reg, bit);
        else
            this.emit("Error", code, reg, bit, info);
    }
